...
RUN apk update && apk upgrade && apk add --no-cache curl tar wget gnupg tzdata \
  make gcc g++ python linux-headers binutils-gold libstdc++ openssl-dev
...
RUN mkdir -p /usr/src/perl && cd /usr/src/perl \
  ...
  && chmod +x cpanm \
  && ./cpanm --no-interactive App::cpanminus LWP::UserAgent HTTP::Date PerlIO::utf8_strict POSIX::strftime::Compiler Digest::SHA Module::Signature LWP::Protocol::https \
  && cd / \
